...
function ParticipantTab({
      ...
      >
        ...
        <Grid item>
          <div style={{display: 'flex'}}>
            ...
            { (
              <PinBtn
                id={"priority-toggle-" + streamId}
                sx={{minWidth: "unset", pt: 1, pb: 1, position: "relative"}}
                onClick={() => togglePriorityParticipant(streamId)}
              >
                {priorityParticipants.includes(streamId) && (
                    <Typography
                        variant="caption"
                        sx={{
                    ...
                    >
                        {priorityParticipants.indexOf(streamId) + 1}
                    </Typography>
                )}
                <SvgIcon size={28} name={priorityParticipants.includes(streamId) ? "eye" : "eye-off"} color={theme.palette?.participantListIcon?.primary}/>
              </PinBtn>
            )}
  ...
  const sortedParticipants = React.useMemo(() => {
    ...
    Object.entries(pagedParticipants).forEach(([streamId, broadcastObject]) => {
      if (publishStreamId !== streamId) {
        if (priorityParticipants.includes(streamId)) {
          prioritized.push([streamId, broadcastObject]);
        } else {
          others.push([streamId, broadcastObject]);
        }
      }
    });

    prioritized.sort((a, b) => priorityParticipants.indexOf(a[0]) - priorityParticipants.indexOf(b[0]));

    // Default sort for others, e.g., by name
    others.sort((a, b) => a[1].name.localeCompare(b[1].name));

    return [...prioritized, ...others];
  }, [pagedParticipants, priorityParticipants, publishStreamId]);
    ...
    <Grid container sx={{mt: 1}} style={{flexWrap: 'nowrap', flex: 'auto', overflowY: 'auto'}}>
      ...
      >
        ...
        <Stack id="participant-scroll" style={{flexWrap: 'nowrap', flex: 'auto', overflowY: 'scroll'}}
          ...
          {sortedParticipants.map(([streamId, broadcastObject]) => {
